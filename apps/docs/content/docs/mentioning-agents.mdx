...
3. A comment that matches neither case goes to the issue's agent assignee; when the assignee is a squad, the leader handles it. This applies to top-level comments and to plain replies in a thread that has no agent owner alike, so an issue assigned after it was created still forwards later replies to the assignee.
